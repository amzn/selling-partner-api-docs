...
销售伙伴 API 端点与特定 AWS 区域相关联。AWS 区域非常重要，因为它是凭证范围的一部分，是在调用销售伙伴 API 时计算签名所必需的。有关更多信息，请参阅[凭据范围](#凭证范围)。
...
在我们将您注册为开发者后，您可以[注册您的销售伙伴 API 应用程序](#注册您的销售伙伴-api-应用程序)。要查看您的开发者信息，请参阅[查看您的开发者信息](#查看您的开发者信息)。
...
**重要说明。** 如果您使用 [AWS Lambda](https://aws.amazon.com/lambda/)、[Amazon EC2](https://aws.amazon.com/ec2/) 和 [Amazon ECS](https://aws.amazon.com/ecs/) 等服务在 AWS 上托管您的销售伙伴 API 应用程序，则您的权限配置将会有所不同。有关更多信息，请参阅[在 AWS 服务上托管您的销售伙伴 API 应用程序](#在-aws-服务上托管您的销售伙伴-api-应用程序)。
...
创建 IAM 用户以获取 AWS 密钥来验证对销售伙伴 API 的调用。我们建议专门为此创建一个新的 IAM 用户。使用 IAM 用户以承担您在[步骤 4：创建 IAM 职权](#步骤-4创建-iam-职权)中创建的 IAM 职权。
...
5. 在**设置权限**页面上，接受默认值，然后单击**下一步： 标签。** 您将在[创建 IAM 职权](#步骤-4创建-iam-职权)时设置权限。

6. 在**添加标签（可选）**页面上，添加可选标签（如果需要），然后单击**下一步： 查看**按钮。

7. 在**查看**页面上，查看您所做的选择。您可以忽略**此用户没有权限**警告。您将在[创建 IAM 职权](#步骤-4创建-iam-职权)时设置权限。当您准备好继续时，请单击**创建用户**按钮。
...
9. 单击**关闭**。在**用户名**列中，单击您的新 IAM 用户并记录用户 ARN。您将在[步骤 4：创建 IAM 职权](#步骤-4创建-iam-职权)中需要它。

有关更多信息，请参阅 AWS 文档中的[在您的 AWS 账户中创建 IAM 用户](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_users_create.html)。

## 步骤 3。创建 IAM 策略

此 IAM 策略定义了对销售伙伴 API 进行调用的权限。您将把它附加到您在[步骤 4：创建 IAM 职权](#步骤-4创建-iam-职权)中创建的 IAM 职权。
...
创建 IAM 职权，该职权信任您在[步骤 2：创建 IAM 用户](#步骤-2创建-iam-用户)中创建的 IAM 用户并具有调用销售伙伴 API 的权限。
...
4. 在**账户 ID** 框中，输入您在[步骤 2：创建 IAM 用户](#步骤-2创建-iam-用户)中创建 IAM 用户时所使用的 AWS 账户的账户编码，然后单击**下一步： 权限**按钮。

5. 在**附加权限策略**页面上的**策略名称**下，选择您在[步骤 3：创建 IAM 策略](#步骤-3创建-iam-策略)中创建的策略，然后单击**下一步： 标签。**
...
9. 保存您的职权 ARN。您将需要它：
   ...
   1. 当您[注册您的应用程序](#步骤-6注册您的应用程序)时。

   2. 在[步骤 5 中。您的应用程序用 LWA 授权码交换 LWA 刷新令牌](#步骤-4您的应用程序用-lwa-授权码交换-lwa-刷新令牌)。
...
2. 在左侧的导航窗格中，单击**用户**，然后单击您在[步骤 2.创建 IAM 用户](#步骤-2创建-iam-用户)中创建的用户。

3. 在左侧的导航窗格中，单击**用户**，然后单击要向其添加 AWS STS 策略的用户。在此工作流程中，选择您在[步骤 2：创建 IAM 用户](#步骤-2创建-iam-用户)中创建的用户。您可以为其他使用案例选择不同的 IAM 用户。
...
10. 在**添加 ARN** 对话框中，输入[步骤 4：创建 IAM 职权](#步骤-4创建-iam-职权)中的职权 ARN（在**指定职权的 ARN** 框中输入），单击**添加**，然后单击**查看策略**按钮。
...
1. 使用您用于[注册成为开发者](#注册为开发者)的凭据登录卖家平台。
...
[注册销售伙伴 API 应用程序](#注册为开发者)后，您可以登录开发者平台以查看您的开发者信息。
...
3. 单击所需应用程序的 **LWA 凭据**下的**查看**。
   ...
   此时将显示该应用程序的 LWA 客户端编码和客户端密钥。您将需要这些凭证来请求 LWA 访问令牌。有关更多信息，请参阅[步骤 1：请求“使用亚马逊账户登录”访问令牌](#步骤-1请求使用亚马逊账户登录访问令牌)。
...
- [商城应用商店工作流程](#商城应用商店工作流程)。从商城应用商店详情页面启动的 OAuth 授权工作流程。

- [网站工作流程](#网站工作流程)。从您自己的网站启动的 OAuth 授权工作流程。
...
如果您正在为自己的销售账户开发应用程序，您可以自行授权。有关更多信息，请参阅[自行授权](#自行授权-1)。
...
- 测试工作流程不是从商城应用商店详情页面开始，而是从卖家直接导航到应用程序的 OAuth 授权 URI 开始。您可以与合作的可信卖家配合进行测试，也可以使用您自己的销售账户凭证自行测试工作流程。OAuth 授权 URI 必须包含 `version=beta` 参数，指示工作流程授权处于草稿状态的应用程序。当卖家导航到 OAuth 授权 URI 时，工作流程将从 [步骤 2：卖家同意授权您的应用程序](#步骤-2卖家同意授权您的应用程序)继续。

   **注意：** 如果您有多个区域 OAuth 授权 URI，请务必为卖家提供与他们的销售区域相对应的 OAuth 授权 URI。

- 您的应用程序将 `version=beta`参数添加到亚马逊回调 URI中 [步骤 3：卖家登录您的网站](#步骤-3卖家登录您的网站)。这将生成可授权处于“草稿”状态的应用程序的工作流程。

完成工作流程测试后，请对其进行更新，使其不再将 `version=beta`参数添加到亚马逊回调 URI中（[步骤 3：卖家登录您的网站](#步骤-3卖家登录您的网站)）。这使其成为一个生产工作流程。现在，任何卖家都可以从商城应用商店的详情页面开始授权您已发布的应用程序。

生产工作流程从[步骤 1：卖家从商城应用商店启动授权](#步骤-1卖家从商城应用商店启动授权)开始。
...
| **state** | 您在上一步中传递的状态值。 |
| **selling\_partner\_id** | 授权您的应用程序的卖家的卖家编号。 |
| **mws\_auth\_token** | 您在为调用亚马逊商城网络服务创建查询字符串时使用的 **MWSAuthToken** 值。只有当卖家授权混合销售伙伴 API 应用程序时，才会传递 mws\_auth\_token 参数。有关更多信息，请参阅[混合销售伙伴 API 应用程序](#混合销售伙伴-api-应用程序)。 |
| **spapi\_oauth\_code** | 您用来交换 LWA 刷新令牌的“使用亚马逊账户登录”(LWA) 授权码。有关更多信息，请参阅[步骤 5：您的](#步骤-5您的应用程序用-lwa-授权码交换-lwa-刷新令牌) [应用程序用 LWA 授权码交换 LWA](#步骤-5您的应用程序用-lwa-授权码交换-lwa-刷新令牌) [刷新令牌](#步骤-5您的应用程序用-lwa-授权码交换-lwa-刷新令牌)。 |
...
| **code** | 您在[步骤 4：亚马逊向您发送授权信息](#步骤-4亚马逊向您发送授权信息)中收到的 LWA 授权码。 |
| **redirect\_uri** | 您的应用程序的重定向 URI。 |
| **client\_id** | 您的 LWA 凭证的一部分。要获得此值，请参阅[查看您的开发者信息](#查看您的开发者信息)。 |
| **client\_secret** | 您的 LWA 凭证的一部分。要获得此值，请参阅[查看您的开发者信息](#查看您的开发者信息)。 |
...
| **参数** | **描述** |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **access\_token** | 授权您的应用程序代表卖家采取某些操作的令牌。请参阅[连接到销售伙伴 API](#连接到销售伙伴-api)。 |
| **token\_type** | 返回的令牌类型。应该是 bearer。 |
| **expires\_in** | 访问令牌失效之前的秒数。 |
| **refresh\_token** | 可以交换为新访问令牌的长期令牌。请参阅[连接到销售伙伴 API](#连接到销售伙伴-api)。 |
```http
HTTP/l.l 200 OK
...
LWA 刷新令牌是您交换 LWA 访问令牌的长期令牌，它必须包含在销售伙伴 API 的每个请求中。发出访问令牌后，它的有效期为一小时。相同的访问令牌可用于多个 API 调用，直到它过期。请参阅[连接到销售伙伴 API](#连接到销售伙伴-api)。

您的应用程序现在已获得授权，可以代表卖家调用销售伙伴 API。

### 适用于混合销售伙伴 API 应用程序

如果在[步骤 4：亚马逊向您发送授权信息](#步骤-4亚马逊向您发送授权信息)中返回了 MWS 授权令牌，您的应用程序也有权代表卖家调用亚马逊商城网络服务。有关更多信息，请参阅[混合销售伙伴 API 应用程序](#混合销售伙伴-api-应用程序)。

## 网站工作流程

网站工作流是从您自己的网站启动的 OAuth 授权工作流程。卖家登录您的网站，然后单击您配置的用以启动授权的“授权”按钮。有关更多信息，请参阅[步骤 0：设置 OAuth 授权 URI](#步骤-0设置-oauth-授权-uri)。
...
- 您的应用程序将 `version=beta` 参数添加到[步骤 1：卖家从您的网站启动授权](#步骤-1卖家从您的网站启动授权)中的 OAuth 授权 URI。这将生成可授权处于“草稿”状态的应用程序的工作流程。

完成工作流程测试后，请更新它，使其不再将 `version=beta` 参数添加到[步骤 1：卖家从您的网站启动授权](#步骤-1卖家从您的网站启动授权)中的 OAuth 授权 URI。这使其成为一个生产工作流程。现在，任何卖家都可以从自己的网站开始授权您已发布的应用程序。

生产工作流程从[步骤 0：设置 OAuth 授权 URI ](#步骤-0设置-oauth-授权-uri)开始。
...
在应用程序网站上设置一个“授权”按钮（或类似的内容），卖家可以单击该按钮启动应用程序的授权。当卖家单击该按钮时，您的网站将 OAuth 授权 URI 加载到浏览器中，卖家将被重定向到卖家平台登录页面。[注册您的应用程序](#步骤-6注册您的应用程序)时，您将获得 OAuth 授权 URI。
...
2. 卖家单击您在[步骤 0：设置 OAuth 授权 URI ](#步骤-0设置-oauth-授权-uri)开始。如果您有多个区域“授权”按钮，请确保卖家被定向到与其所在销售区域相对应的按钮。
...
| **参数** | **描述** |
| ------------------------ | ---------------------- |
| **state** | [步骤 1：卖家从](#步骤-1卖家从您的网站启动授权) [您的网站启动授权](#步骤-1卖家从您的网站启动授权)中的 state 值。 |
| **selling\_partner\_id** | 授权您的应用程序的卖家的卖家编号。 |
| **mws\_auth\_token** | 您在为调用亚马逊商城网络服务创建查询字符串时使用的 **MWSAuthToken** 值。只有当卖家授权混合销售伙伴 API 应用程序时，才会传递 mws\_auth\_token 参数。有关更多信息，请参阅[混合销售伙伴 API 应用程序](#混合销售伙伴-api-应用程序)。 |
| **spapi\_oauth\_code** | 您用来交换 LWA 刷新令牌的“使用亚马逊账户登录”(LWA) 授权码。有关更多信息，请参阅[步骤 4：您的[ ](#步骤-4您的应用程序用-lwa-授权码交换-lwa-刷新令牌)应用程序用 LWA 授权码交换 LWA 刷新](#步骤-4您的应用程序用-lwa-授权码交换-lwa-刷新令牌) [令牌](#步骤-4您的应用程序用-lwa-授权码交换-lwa-刷新令牌)。 |
...
<td>您在<a href="#步骤-3亚马逊向您发送授权信息">步骤 3：亚马逊向您发送授权</a> <a href="#步骤-3亚马逊向您发送授权信息">信息</a>中收到的 LWA 授权码。</td>
</tr>
<tr class="odd">
<td><strong>redirect_uri</strong></td>
...
<td>您的 LWA 凭证的一部分。要获得此值，请参阅<a href="#查看您的开发者信息">查看您的开发者信息</a>。</td>
</tr>
<tr class="odd">
...
如果在[步骤 3：亚马逊向您发送授权信息](#步骤-3亚马逊向您发送授权信息)中返回了 MWS 授权令牌，您的应用程序也有权代表卖家调用亚马逊商城网络服务。有关更多信息，请参阅[混合销售伙伴 API 应用程序](#混合销售伙伴-api-应用程序)。

# 自行授权

您可以在开发者平台中自行授权您的应用程序。在这样做之前，您必须[注册您的销售伙伴 API 应用程序](#自行授权-1)。

要实施完整的 OAuth 授权工作流程，请参阅[授权销售伙伴 API 应用程序](#授权销售伙伴-api-应用程序)。

1. **要自行授权您的应用程序**，请使用您用于[注册成为开发者](#注册为开发者)的凭据登录卖家平台。
...
刷新令牌是一种长期令牌，可用来交换短期访问令牌。对销售伙伴 API 的每个请求都必须包含访问令牌。发出访问令牌后，它的有效期为一小时。相同的访问令牌可用于多个 API 调用，直到它过期。有关更多信息，请参阅[步骤 1：请求“使用亚马逊账户登录”访问令牌](#步骤-1请求使用亚马逊账户登录访问令牌)。
...
生成软件 SDK 后，您可以使用它调用销售伙伴 API。请参阅[使用生成的 Java SDK 连接到销售伙伴 API](#连接到销售伙伴-api-using-a-generated-java-sdk)。

# 使用生成的 Java SDK 连接到销售伙伴 API

您必须首先注册您的应用程序，并且必须获得卖家授权，之后才能将该应用程序连接到销售伙伴 API。请参阅[注册您的销售伙伴 API 应用程序](#注册为开发者)和[授权销售伙伴 API 应用程序](#授权销售伙伴-api-应用程序)。

这些说明介绍了如何使用生成的 Java SDK 进行调用。该 SDK 提供了用于配置 LWA 和 AWS 凭证的类，并使用这些类来交换 LWA 令牌以及为您签署请求。有关更多信息，请参阅[通过 LWA 令牌交换和身份验证生成 Java SDK](#通过-lwa-令牌交换和身份验证生成-java-sdk)。
...
<td>您的 AWS 访问密钥 ID，来自<a href="#步骤-4创建-iam-职权">步骤 2：创建 IAM 用户</a>。
</td>
<td>是
</td>
</tr>
...
<td>您的 AWS 私有访问密钥，来自<a href="#步骤-4创建-iam-职权">步骤 2：创建 IAM 用户</a>。
</td>
<td>是
</td>
</tr>
...
<td>您在<a href="#步骤-4创建-iam-职权">步骤 4：创建 IAM 职权</a>中创建的 IAM 职权的 ARN。</td>
<td>是</td>
</tr>
<tr class="odd">
...
1. 将 AWS STS 策略附加到您用于设置亚马逊 ECS 的 IAM 用户。有关说明，请参阅[步骤 5：向 IAM 用户添加 AWS 安全令牌服务策略](#步骤-5向-iam-用户添加-aws-安全令牌服务策略)以获取说明。

2. 附加一个策略，该策略定义了为亚马逊 EC2 实例承担的职权调用销售伙伴 API 的权限。有关说明，请参阅[步骤 3：创建 IAM 策略](#步骤-3创建-iam-策略)。
...
- 附加一个策略，该策略定义了为 Lambda 函数承担的职权调用销售伙伴 API 的权限。有关说明，请参阅[步骤 3：创建 IAM 策略](#步骤-3创建-iam-策略)。
...
<td>您IAM的ARN，来自<a href="#步骤-4创建-iam-职权">步骤 2：创建 IAM 用户</a>。
</td>
<td>是
</td>
</tr>
...
<td>您的 LWA 客户端标识符。有关更多信息，请参阅<a href="#查看您的开发者信息">查看</a><a href="#查看您的开发者信息">您的开发者信息</a>。</td>
<td>是</td>
</tr>
<tr class="even">
<td><strong>clientSecret</strong></td>
<td>您的 LWA 客户密钥。有关更多信息，请参阅<a href="#查看您的开发者信息">查看</a><a href="#查看您的开发者信息">您的开发者信息</a>。</td>
<td>是</td>
</tr>
<tr class="odd">
<td><strong>refreshToken</strong></td>
<td>LWA 刷新令牌。当卖家授权您的应用程序时，可以获得该值。有关更多信息，请参阅<a href="#授权销售伙伴-api-应用程序">授权销售</a><a href="#授权销售伙伴-api-应用程序">伙伴 API 应用程序</a>。</td>
<td><p>否。如果您在以下步骤中调用的操作需要卖家授权，请包括刷新令牌。所有非<a href="#免授权操作-1">免授权操作</a>的操作都需要卖家授权。如果您包含 refreshToken，请不要包含 withScopes。</p></td>
</tr>
<tr class="even">
...
<td>否。如果您在以下步骤中调用的操作是<a href="#免授权操作-1">免授权</a><a href="#免授权操作-1">操作</a>，则包括 withScopes。如果您包含 withScopes，请不要包含 refreshToken。</td>
</tr>
<tr class="odd">
...
虽然生成的客户端库可以帮助您调用销售伙伴 API，但它不包含用于 LWA 令牌交换和身份验证的代码。关于这一点，请参阅[步骤 1：请求“使用亚马逊账户登录”访问令牌](#步骤-1请求使用亚马逊账户登录访问令牌)和[步骤 4：创建并[ ](#步骤-4创建并签署您的请求)签署您的请求](#步骤-4创建并签署您的请求)。或者，对于包含 LWA 令牌交换和身份验证的 SDK，请参阅[通过 LWA 令牌交换](#通过-lwa-令牌交换和身份验证生成-java-sdk)[和身份验证生成 Java SDK](#通过-lwa-令牌交换和身份验证生成-java-sdk)。
...
生成客户端库后，您可以使用它来帮助您调用销售伙伴 API。请参阅[连接到销售伙伴 API](#连接到销售伙伴-api)。

# 连接到销售伙伴 API

您必须首先注册您的应用程序，并且必须获得卖家授权，之后才能将该应用程序连接到销售伙伴 API。请参阅[注册您的销售伙伴 API 应用程序](#注册为开发者)和[授权销售伙伴 API 应用程序](#授权销售伙伴-api-应用程序)。

这些说明向您展示了调用销售伙伴 API 的步骤。有关构建销售伙伴 API URI 并向其添加标头的帮助，请参阅[生成 Java 客户端库](#Generating_a_Java_client_library)。有关更完整的解决方案（包括用于交换 LWA 令牌和身份验证的代码），请参阅[通过 LWA 令牌交换](#通过-lwa-令牌交换和身份验证生成-java-sdk)[和身份验证生成 Java SDK](#通过-lwa-令牌交换和身份验证生成-java-sdk)。
...
<td><p>请求的访问授权类型。值：</p>
<ul>
<li><p><em>refresh_token</em>。这用于调用需要卖家授权的操作。所有非<a href="#免授权操作-1">免授权操作</a>的操作都需要卖家授权。指定此值时，请包括 <em>refresh_token</em> 参数。</p></li>
<li><p><em>client_credentials</em>。这用于调用<a href="#免授权操作-1">免授权操作</a>。指定此值时，请包括 <code>scope</code> 参数。</p></li>
</ul></td>
<td>是</td>
</tr>
<tr class="even">
<td><strong>refresh_token</strong></td>
<td>LWA 刷新令牌。当卖家授权您的应用程序时，可以获得该值。有关更多信息，请参阅<a href="#授权销售伙伴-api-应用程序">授权销售伙伴 API 应用程序</a>。</td>
...
<td>否。包含 scope 以调用 <a href="#免授权操作-1">免授权</a><a href="#免授权操作-1">操作</a>。如果包含 scope，请不要包含 refresh_token。</td>
</tr>
<tr class="even">
<td><strong>client_id</strong></td>
<td>注册应用程序时获取该值。请参阅<a href="#查看您的开发者信息">查看您的开发者信息</a>。</td>
<td>是</td>
</tr>
<tr class="odd">
<td><strong>client_secret</strong></td>
...
将标头添加到您在[步骤 2：构建销售伙伴 API URI](#步骤-2构建销售伙伴-api-uri) 中构建的 URI。以下是您在销售伙伴 API 的请求中包含的 HTTP 标头：
...
| x-amz-access-token | LWA 访问令牌。请参阅[步骤 1：请求“使用亚马逊账户登录”访问令牌](#步骤-1请求使用亚马逊账户登录访问令牌)。 |
| x-amz-date | 请求的日期和时间。 |
| user-agent | 您的应用程序名称和版本号、平台和编程语言。这些内容可帮助亚马逊诊断和修复您可能遇到的服务问题。请参阅[在所有请求中](#在所有请求中包含-user-agent-标头)[包含 User-Agent 标头](#在所有请求中包含-user-agent-标头)。 |
...
```
要签署销售伙伴 API 的请求，请参阅[步骤 4：创建并签署您的请求](#步骤-4创建并签署您的请求)。

## 步骤 4。创建并签署您的请求

销售伙伴 API 使用 AWS [签名版本 4 签名流程](https://docs.aws.amazon.com/general/latest/gr/signature-version-4.html)对请求进行身份验证。当您向销售伙伴 API 发送 HTTP 请求时，您需要对请求进行签名，以便亚马逊能够识别谁发送了这些请求。您可以使用 AWS 访问密钥签署请求，该密钥包括访问密钥 ID 和访问密钥。有关获取 AWS 访问密钥的信息，请参阅[步骤 2：创建 IAM 用户](#步骤-4创建-iam-职权)中创建的用户。
...
1. 创建规范请求
   ...
   - 请参阅[步骤 3：有关创建规范请求时开始的未签名请求示例，请将标头添加到 URI](#步骤-3将标头添加到-uri)。
...
凭证范围是您在签署销售伙伴 API 的请求时创建的“要签名的字符串”的组成部分。请参阅[创建并签署您的请求](#步骤-4创建并签署您的请求)。
...
有关更多信息，请参阅[步骤 4：创建并签署您的请求](#步骤-4创建并签署您的请求)。
...
| **组成部分** | **描述** |
| ------------------------------ | ---------|
| 用于签名的算法 | 整个签名过程中使用的哈希算法。销售伙伴 API 需要 SHA-256。您可以在[步骤 4：创建并签署您的请求](#步骤-4创建并签署您的请求)中指定此项。 |
| Credential | 您的 AWS 访问密钥编码加上[凭证范围](#凭证范围)。您可以在[步骤](#步骤-4创建-iam-职权) [3：创建 IAM 用户](#步骤-4创建-iam-职权)中获取 AWS 访问密钥编码。 |
| SignedHeaders | 签名请求中包含的所有 HTTP 标头的列表。有关示例，请参阅[步骤 3：将标头添加到 URI](#步骤-3将标头添加到-uri)。 |
| Signature | 在[步骤 4：创建并签署您的请求](#步骤-4创建并签署您的请求)中计算的签名。 |
...
免授权操作是您可以在没有销售伙伴明确授权的情况下调用的操作。这意味着，当您在调用免授权操作之前[请求“使用亚马逊账户登录”访问令牌](#步骤-1请求使用亚马逊账户登录访问令牌)时，您无需提供刷新令牌。相反，您可以使用 **scope** 参数来提供 LWA 授权的范围。如果您使用生成的 Java SDK（请参阅[使用生成的 Java SDK 连接到销售伙伴 API](#使用生成的-java-sdk-连接到销售伙伴-api)）调用免授权操作，请在配置 LWA 凭证时使用 **withScopes** 参数为 LWA 授权设置一个或多个范围。
...
2. 使用您用于[注册成为开发者](#注册为开发者)的凭据登录卖家平台。
...
创建混合应用程序后，您可以设置和测试 OAuth 授权工作流程。有关更多信息，请参阅[授权销售伙伴 API 应用程序](#授权销售伙伴-api-应用程序)。
...
销售伙伴 API 提供了一个沙箱环境，允许您在不影响生产数据或触发真实情况下测试应用程序。对销售伙伴 API 进行沙箱调用与进行生产调用相同，不同之处在于您要将调用指向[销售伙伴 API 沙箱端点](#销售伙伴-api-沙箱端点)。调用沙箱端点会返回所有销售伙伴 API 的静态模拟响应。您可以在 Swagger 模型 JSON 文件中为要调用的 API 引用这些模拟响应。有关更多信息，请参阅[如何对销售伙伴 API 进行沙箱调用](#如何对销售伙伴-api-进行沙箱调用)。
...
2. 将您的调用指向[销售伙伴 API 沙箱端点](#销售伙伴-api-沙箱端点)之一。

   您应收到一个响应，该响应与 JSON 文件的 x-amazon-spds-sandbox-behaviors 对象中包含的有效负载对象匹配。

## 销售伙伴 API 沙箱端点

销售伙伴 API 具有针对北美、欧洲和远东销售区域的沙箱端点。有关更多信息，请参阅[销售伙伴 API 沙箱](#混合销售伙伴-api-应用程序)。
...
- 销售伙伴 API 授权利用 LWA，即亚马逊对 OAuth 2.0 的实施。使用此模型，您无需按照亚马逊 MWS 的要求手动交换身份验证令牌。请参阅[授权您的销售伙伴 API 应用程序](#网站工作流程)。
...
- 销售伙伴 API 允许您使用 AWS Identity and Access Management (IAM) 直接获取和管理自己的身份验证凭证。借助亚马逊 MWS，您可使用专用注册工作流程接收亚马逊提供的身份验证凭证，并通过与亚马逊 MWS 支持部门联系来获得新凭证。请参阅[步骤 2：创建 IAM 用户](#步骤-4创建-iam-职权)。

- 销售伙伴 API 使用 AWS 签名版本 4 进行身份验证。亚马逊 MWS 使用签名版本 2。请参阅[步骤 4：创建并签署您的请求](#步骤-4创建并签署您的请求)。
